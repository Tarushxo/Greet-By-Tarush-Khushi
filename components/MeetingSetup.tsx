...
const MeetingSetup = ({setisSetupComplete}:{setisSetupComplete : (value: boolean) => void}) => {
    ...
    if (!call) {
        throw new Error('Use call must be within StreamCall component');
    }
    ...
    <div className='flex h-screen w-full flex-col items-center gap-3 justify-center text-white'>
        ...
        <div className='flex h-16 items-center justify-center gap-3'>
            <label className='flex items-center justify-center gap-2 font-medium'>
                <input type='checkbox' checked={isMicCamToggledOn} onChange={(e) => setisMicCamToggledOn(e.target.checked)}/>Join with Mic & Cam turned off
            </label>
            <DeviceSettings />
        </div>
